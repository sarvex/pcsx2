...
def run_regression_test(runner, dumpdir, renderer, upscale, renderhacks, parallel, gspath):
    ...
    args.extend(
        ["-logfile", os.path.join(real_dumpdir, "emulog.txt"), "-loop", "2"]
    )
...
def run_regression_tests(runner, gsdir, dumpdir, renderer, upscale, renderhacks, parallel=1):
    paths = glob.glob(f"{gsdir}/*.*", recursive=True)
    ...
    if parallel <= 1:
        ...
        for completed, _ in enumerate(pool.imap_unordered(func, gamepaths, chunksize=1), start=1):
